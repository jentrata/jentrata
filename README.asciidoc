...
. Download and unpack jentrata-<version>.tar.gz
. Then run 
[source,bash]
----
...
----
You should see that the *Jentrata :: ebms3 - AS4 Conformance Profile* bundles is in the Created state

By default Jentrata exposes a single http endpoint http://<hostname>:8081/jentrata/ebms/inbound for ebMS AS4 support.
